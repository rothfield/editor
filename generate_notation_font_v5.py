...
def create_dot_variants_flattened(input_font_path, output_font_path, pua_start=0xE000):
    ...
    # Now flatten ALL the custom glyphs to convert references to outlines
    print(f"\nFlattening all {created} custom glyphs...")
    for cp in range(pua_start, pua_start + created):
        g = font[cp]
        if g and len(g.references) > 0:
            # Create a temporary composite, then flatten it back
            g.correctDirection()

    print("Flattening font structure...")
    # Flatten with removeOutlines=False to preserve the structure
    font.flatten()
    ...
    try:
        ...
        print("✓ Font generated successfully with flattened composite glyphs!")
        return True
