...
fn generate_font_constants(atoms: &serde_yaml::Value, out_dir: &PathBuf) {
    ...
    const ACCIDENTAL_TYPES: usize = 6; // natural, flat, half-flat, double-flat, sharp, double-sharp
    const OCTAVE_VARIANTS: usize = 5;  // 5 octave patterns: 0, -2, -1, +1, +2
...
fn generate_system_lookup_table(
    ...
    for (pitch_idx, pitch_code) in pitch_codes.iter().enumerate() {
        ...
        // Get the base character for this degree
        let base_char = if let Some(char_entry) = characters.get(degree) {
...
fn generate_fontspec_json(atoms: &serde_yaml::Value, out_dir: &PathBuf) {
    ...
    // Also write to src/fontgen/fontspec.json for Python consumption
    let python_fontspec_path = PathBuf::from("src/fontgen/fontspec.json");
    fs::write(&python_fontspec_path, &json_str)
        .expect("Failed to write src/fontgen/fontspec.json");
}
