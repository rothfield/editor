...
pub fn derive_kind(cp: u32) -> crate::models::elements::ElementKind {
    ...
    // Only digits 1-7 are unambiguously pitched (Number system)
    // Letters like 'F' must be converted to PUA by parse_note() to be pitched
    // This ensures 'F' is Text in Number system, but '1' is still PitchedElement
    if let Some(ch) = char::from_u32(cp) {
        // Number system digits are unambiguously pitched
        if ch >= '1' && ch <= '7' {
            return ElementKind::PitchedElement;
        }
        // All other alphanumeric (letters, 0, 8, 9) are Text
...
fn generate_system_lookup_table(
    ...
    // Add ASCII characters for all systems in pitch_from_glyph_*()
    // This allows pitch_from_glyph(ch, system) to recognize ASCII chars when
    // the pitch system is explicitly specified.
    // NOTE: derive_kind() does NOT use these - it has separate logic that treats
    // ASCII letters as Text (since it has no pitch system context)
    for (pitch_idx, pitch_code) in pitch_codes.iter().enumerate() {
...
fn generate_superscript_tables(atoms: &serde_yaml::Value, out_dir: &PathBuf) {
    let code = String::from(r#"/// Auto-generated superscript lookup tables from atoms.yaml at compile time
/// DO NOT EDIT - This file is generated by build.rs
///
