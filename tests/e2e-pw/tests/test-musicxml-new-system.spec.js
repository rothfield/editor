...
test('Type 1 + Enter + 2 should create solo-style single part with two measures (new-system break)', async ({ page }) => {
  await page.goto('/');
  ...
  // Verify solo-style structure: ONE part with TWO measures
  expect(musicxml).toContain('<score-part id="P1">');
  expect(musicxml).not.toContain('<score-part id="P2">'); // Should only have P1
  expect(musicxml).toContain('<measure number="1">');
  expect(musicxml).toContain('<measure number="2">');
  expect(musicxml).toContain('<print new-system="yes"/>'); // System break on measure 2

  // Verify measure 1 does NOT have <print new-system>
  ...
  // Verify measure 2 DOES have <print new-system>
