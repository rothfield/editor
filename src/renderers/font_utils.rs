...
//! and accidentals to Private Use Area (PUA) codepoints in the NotationFont.ttf.
//!
//! The NotationFont.ttf contains two sets of glyphs:
//!
//! **Octave Variants** (188 glyphs - 47 chars × 4 variants):
...
//!
//! Each character has 4 octave shift variants:
...
//!
//! **Accidental Variants** (47 glyphs - 47 chars × 1 accidental each):
//! - Sharp glyphs (1#, 2#, C#, c#, S#, etc.)
//! - Flat glyphs (1b, 2b, Cb, cb, Sb, etc.) - generated from atoms.yaml
//! - Double sharp glyphs (1##, 2##, C##, etc.) - future expansion
//! - Double flat glyphs (1bb, 2bb, Cbb, etc.) - future expansion
//!
//! **Note:** All constants are generated at compile-time from tools/fontgen/atoms.yaml
...
/// - pua_start: Starting codepoint for octave variants (0xE000)
/// - chars_per_variant: Number of variants per character (4)
/// - accidental_pua_start: Starting codepoint for accidentals (0xE1F0)
/// - symbols_pua_start: Starting codepoint for symbols (0xE220)
...
pub fn get_font_config() -> JsValue {
    ...
    struct FontConfig {
        ...
        accidental_pua_start: u32,
        symbols_pua_start: u32,
    ...
    let config = FontConfig {
        ...
        accidental_pua_start: ACCIDENTAL_PUA_START,
...
pub fn get_sharp_glyph_codepoint(base_char: char, accidental: u8) -> char {
    // Natural (no accidental) means use base character
    if accidental == 0 || accidental == b'n' {
        return base_char;
    }

    // Currently only sharp accidentals are supported (in the generated font)
    // TODO: Support flat, double sharp, double flat when they are added to font
    match accidental {
        1 | b's' => {
            // Sharp accidental: use PUA glyph
            let char_index = match ALL_CHARS.find(base_char) {
                Some(idx) => idx as u32,
                None => return base_char, // Unknown character, return as-is
            };

            // Calculate the codepoint: sequential allocation starting at 0xE1F0
            // U+E1F0 = '1#', U+E1F1 = '2#', ..., U+E21E = 'T#'
            let codepoint = ACCIDENTAL_PUA_START + char_index;

            // Convert to character, fallback to base if conversion fails
            char::from_u32(codepoint).unwrap_or(base_char)
        }
        2 | b'b' | 3 | b'x' | 4 | b'y' => {
            // Flat, double sharp, double flat not yet in font
            // Render as base character for now
            base_char
        }
        _ => base_char, // Unknown accidental code
    }
}
