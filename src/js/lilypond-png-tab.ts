...
interface LilyPondRenderer {
  renderNow: (source: string, options?: any) => Promise<void>;
}
...
class LilyPondPngTab {
  ...
  private lilypondRenderer: LilyPondRenderer;
  private container: HTMLElement | null = null;
  private renderArea: HTMLDivElement | null = null;
  private isRendering: boolean = false;

  constructor(editor: Editor, lilypondRenderer: LilyPondRenderer) {
    this.editor = editor;
   ...
   */
  async _performRender(): Promise<void> {
      ...
      await this.lilypondRenderer.renderNow(lilypondSource, {
        ...
        onSuccess: (result) => {
          ...
          if (result.multiPage && result.pages) {
            this.displayMultiPageSVG(result.pages, result.pageCount || 1);
  ...
  getLilyPondSource(): string | null {
    ...
    try {
      ...
      logger.error(LOG_CATEGORIES.WASM, 'LilyPondDisplay: WASM conversion failed', { message: errorMessage });
      return null;
    }
  }
   ...
   */
  displayMultiPageSVG(pagesBase64: string[], pageCount: number): void {
   ...
   */
  displaySVG(base64Data: string): void {
  ...
  private showLoadingIndicator(): void {
    ...
    this.renderArea.innerHTML = `
      <div class="flex flex-col items-center justify-center h-full gap-4">
        ...
        <div class="text-gray-600">Rendering notation...</div>
      </div>
    `;
  }
