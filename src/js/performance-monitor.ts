...
interface PerformanceMetrics {
  typingLatency: PerformanceSample[];
  beatDerivation: PerformanceSample[];
  renderTime: PerformanceSample[];
  focusActivation: PerformanceSample[];
  navigationLatency: PerformanceSample[];
  selectionLatency: PerformanceSample[];
  commandLatency: PerformanceSample[];
...
class PerformanceMonitor {
  ...
  private statsCache: Map<string, MetricStatistics>;
   ...
   */
  endTimer(label: string, category: string = 'commandLatency'): number | null {
   ...
   */
  recordMetric(category: string, value: number): void {
   ...
   */
  private checkPerformanceThresholds(category: string, value: number): void {
   ...
   */
  getStatistics(category: string): MetricStatistics {
    // Check cache first
    const cached = this.statsCache.get(category);
    if (cached) {
      return cached;
    }
   ...
   */
  getAllStatistics(): Record<string, MetricStatistics> {
    const allStats: Record<string, MetricStatistics> = {};
   ...
   */
  updateIndicator(state: string, message: string = ''): void {
