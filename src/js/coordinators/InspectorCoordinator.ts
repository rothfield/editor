...
import type { Document, Cell } from '../../types/wasm.js';
import type { DisplayList } from '../../types/wasm-module.js';
...
interface ExportManager {
  updateIRDisplay(): Promise<void>;
  updateMusicXMLDisplay(): Promise<void>;
  updateLilyPondDisplay(): Promise<void>;
  updateTextDisplay(): Promise<void>;
  updateMarkupDisplay(): Promise<void>;
...
interface DisplayListData {
  title?: string;
  composer?: string;
  lines: DisplayListLine[];
}

interface DisplayListLine {
  line_index: number;
  label?: string;
  text: string;
  cursor_pos?: number | null;
  decoded_glyphs?: DecodedGlyph[];
  lyrics?: OverlayItem[];
  talas?: OverlayItem[];
}
...
interface EditorInstance {
  ...
  osmdRenderer?: OSMDRenderer | null;
  displayList?: DisplayListData | null;
...
export default class InspectorCoordinator {
  ...
  private _hitboxUpdateScheduled: boolean;
  ...
  updateDocumentDisplay(): void {
    ...
    // Update Markup display only if visible
    if (this.editor.ui && this.editor.ui.activeTab === 'markup') {
      this.editor.exportManager.updateMarkupDisplay().catch(err => {
        logger.error(LOG_CATEGORIES.INSPECTOR, 'Failed to update Markup display', { error: err });
      });
    }
   ...
   */
  async forceUpdateAllExports(): Promise<void> {
   ...
   */
  createDisplayDocument(doc: Document | null): Record<string, unknown> {
    if (!doc) return {};

    // Deep clone the document
    const displayDoc = JSON.parse(JSON.stringify(doc)) as Record<string, unknown>;
    ...
    const lines = displayDoc.lines as Array<Record<string, unknown>> | undefined;
      ...
      if (cells && cells.length > 0) {
        ...
        cells.forEach((cell, cellIndex) => {
          ...
          if (hasValidHitbox) {
            ...
            hitboxHTML += `<td class="border border-gray-300 px-2 py-1">${cellWithLayout.x!.toFixed(1)}, ${cellWithLayout.y!.toFixed(1)}, ${cellWithLayout.w!.toFixed(1)}×${cellWithLayout.h!.toFixed(1)}</td>`;
   ...
   */
  formatDisplayList(displayList: DisplayListData | null): string {
    ...
    // Each line
    displayList.lines.forEach((line) => {
