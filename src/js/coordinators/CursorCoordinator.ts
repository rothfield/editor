...
import type { ICursorCoordinator } from '../../types/coordinators.js';
import type { Pos } from '../../types/wasm.js';

export default class CursorCoordinator implements ICursorCoordinator {
  private editor: Editor;
  private _blinkInterval: number | NodeJS.Timeout | null;
   ...
   */
  setCursorPosition(_positionOrRow: number, _col?: number): void {
   ...
   * @param {any} diff - The diff returned from WASM commands
   * @deprecated This function is obsolete - WASM owns cursor state, just call render()
   */
  async updateCursorFromWASM(_diff: any): Promise<void> {
  ...
  getCursorElement(): HTMLElement | null {
    let cursor = document.querySelector('.cursor-indicator') as HTMLElement;
  ...
  startCursorBlinking(): void {
    ...
    if (this._blinkInterval) {
      clearInterval(this._blinkInterval as NodeJS.Timeout);
      this._blinkInterval = null;
    }
  ...
  updateCursorVisualPosition(): void {
    ...
    const cells = this.editor.element.querySelectorAll(
      `[data-line-index="${currentStave}"]`
    );
    ...
    if (cells.length > 0) {
      ...
      const cellContainer = firstCell.closest('.cell-container') as HTMLElement;
      if (cellContainer) {
  ...
  scrollCursorIntoView(): void {
    ...
    const notationLine = cursor.closest('.notation-line') as HTMLElement;
    if (!notationLine) {
    ...
    const lineHeight =
      parseFloat(notationLine.style.height) ||
      notationLine.getBoundingClientRect().height;
    ...
    if (isAboveViewport || isBelowViewport) {
      ...
      if (isAboveViewport) {
        ...
        const lineBottomRelativeToContainer =
          lineContentBottom - containerRect.bottom;
    ...
    if (isLeftOfViewport || isRightOfViewport) {
      ...
      if (isLeftOfViewport) {
        const cursorLeftRelativeToContainer =
          cursorRect.left - containerRect.left;
        targetScrollLeft =
          currentScrollLeft + cursorLeftRelativeToContainer - 20; // 20px padding
      } else if (isRightOfViewport) {
        const cursorRightRelativeToContainer =
          cursorRect.right - containerRect.right;
        targetScrollLeft =
          currentScrollLeft + cursorRightRelativeToContainer + 20; // 20px padding
      }
  ...
  updateCursorPositionDisplay(): void {
    ...
    if (charCount && this.editor.getCurrentLine()) {
      ...
      charCount.textContent = cells.length.toString();
    }
