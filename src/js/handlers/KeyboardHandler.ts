...
interface EditorInstance {
  ...
  showWarning(message: string, options?: { important?: boolean; details?: string }): void;
  getSelection(): Selection | null;
  getDocument(): any;
  render(): Promise<void>;
  renderAndUpdate(): Promise<void>;
  renderStaffNotation(): Promise<void>;
  showTalaDialog(): void | Promise<void>;
...
export class KeyboardHandler {
   ...
   */
  async handleAltCommand(key: string): Promise<void> {
   ...
   */
  private async _applySlurLayered(): Promise<void> {
    ...
    try {
      ...
      this.editor.wasmModule.setSelection(selection.anchor as any, selection.head as any);
   ...
   */
  private async _applyOctaveLayered(target_octave: number): Promise<void> {
    if (!this.editor.isInitialized || !this.editor.wasmModule) {
      return;
    }
   ...
   */
  async handleAltShiftCommand(key: string): Promise<void> {
   ...
   * Remove slur using layered API
   */
  private async _removeSlurLayered(): Promise<void> {
    if (!this.editor.isInitialized || !this.editor.wasmModule) {
      return;
    }
   ...
   */
  async handleCtrlCommand(key: string): Promise<void> {
    ...
    switch (key.toLowerCase()) {
      case 'a':
        ...
        break;
      // Note: case 'c' (Ctrl+C) is intentionally NOT handled here
      // We let native browser copy work on the textarea PUA content
      // "Copy as Markup" is available via Edit menu
      case 'x':
   ...
   */
  async handleShiftCommand(key: string): Promise<void> {
    try {
   ...
   */
  async handleNavigation(key: string): Promise<void> {
