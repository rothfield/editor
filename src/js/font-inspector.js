...
const SUPERSCRIPT_LINE_VARIANTS = [
...
export class FontInspector {
  constructor() {
    this.display = /** @type {HTMLTextAreaElement | null} */ (document.getElementById('font-inspector-display'));
    this.categorySelect = /** @type {HTMLSelectElement | null} */ (document.getElementById('font-inspector-category'));
    this.systemSelect = /** @type {HTMLSelectElement | null} */ (document.getElementById('font-inspector-system'));
    this.sizeSelect = /** @type {HTMLSelectElement | null} */ (document.getElementById('font-inspector-size'));
  ...
  showCharacterInfo(char, position) {
    const codepoint = char.codePointAt(0);
    const hex = `U+${codepoint.toString(16).toUpperCase().padStart(4, '0')}`;
    const dec = codepoint;
    const isPUA = codepoint >= 0xE000;

    let description = this.describeCodepoint(codepoint);
  ...
  describeCodepoint(cp) {
    ...
    if (cp >= 0xF8000) {
      for (const [name, config] of Object.entries(PUA.superscripts)) {
        if (name === 'ascii') {
          if (cp >= config.base && cp < config.base + config.charCount * 16) {
            const offset = cp - config.base;
          ...
          const totalGlyphs = config.chars * config.variantsPerChar * 16;
  ...
  generateBasePitches() {
    let text = '';
    let info = '<strong>Base Pitches</strong> (from atoms.yaml notation_systems)<br>';
    info += 'Natural accidental, octave 0. Click any character for Unicode details.';

    for (const [name, sys] of Object.entries(PUA.systems)) {
      // Show system name and description from atoms.yaml
      text += `${name} (${sys.chars.length} chars): `;
  ...
  generateUnderlines() {
    ...
    const dashU = (variant) => String.fromCodePoint(dashLineBase + variant);
  ...
  generateSuperscripts() {
    ...
    for (let i = 0; i < sys.chars.length; i++) {
      const sourceOffset = i * superConfig.variantsPerChar;
  ...
  generateSymbols() {
    ...
    const fontConfig = window.editor?.wasmModule?.getFontConfig?.();
  ...
  generateExampleBeats() {
    ...
    const noteWithLine = (charIdx, lineVariant) => {
      const noteOffset = charIdx * sys.variantsPerChar;
    ...
    if (superConfig) {
      const grace = (charIdx) => {
        const sourceOffset = charIdx * superConfig.variantsPerChar;
...
export function initFontInspector() {
  ...
  window.fontInspector = inspector;
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initFontInspector);
} else {
  // DOM already loaded, but wait for WASM
  setTimeout(initFontInspector, 100);
}
