...
interface Constraint {
  ...
  description?: string;
  category: string;
  degrees: (string | DegreeConstraint)[];
}
...
type TabName = 'western' | 'raga' | 'maqam' | 'all';

export class ConstraintsDialog {
  private editor: Editor;
  private modal: HTMLElement;
  private closeBtn: HTMLElement;
  private closeBtnFooter: HTMLElement | null;
  ...
  private grids: Record<TabName, HTMLElement | null>;
  private constraints: Constraint[] = [];
  private selectedConstraintId: string | null = null;
  private currentTab: TabName = 'western';
  private searchQuery: string = '';
  private selectedPitchSystem: string = 'Number';
  private pitchSystemSelect: HTMLSelectElement | null;
  ...
  constructor(editor: Editor) {
    ...
    this.searchInput = document.getElementById('constraints-search') as HTMLInputElement;
    this.emptyState = document.getElementById('constraints-empty')!;

    // Tab elements
    this.tabs = this.modal.querySelectorAll<HTMLElement>('.constraints-tab');
    this.tabPanels = this.modal.querySelectorAll<HTMLElement>('.constraints-tab-panel');
    ...
    this.pitchSystemSelect = document.getElementById('constraints-pitch-system-select') as HTMLSelectElement | null;
  ...
  bindEvents(): void {
    ...
    this.tabs.forEach(tab => {
      tab.addEventListener('click', () => {
        const tabName = tab.dataset.tab as TabName;
   ...
   */
  renderTab(tabName: TabName): void {
    ...
    grid.querySelectorAll('.constraints-card').forEach(card => {
      card.addEventListener('click', () => {
        const constraintId = (card as HTMLElement).dataset.constraintId;
        if (constraintId) {
   ...
   */
  filterConstraintsByTab(tabName: TabName): Constraint[] {
    if (tabName === 'all') {
   ...
   */
  getConstraintNotesForDisplay(constraintId: string): string[] {
    try {
      const wasmModule = this.editor.wasmModule;
   ...
   */
  renderCard(constraint: Constraint): string {
   ...
   */
  getConstraintBadges(constraint: Constraint): Badge[] {
    const badges: Badge[] = [];
   ...
   * Select a constraint
   */
  async selectConstraint(constraintId: string): Promise<void> {
    try {
      const wasmModule = this.editor.wasmModule;
   ...
   */
  async clearConstraint(): Promise<void> {
    try {
      const wasmModule = this.editor.wasmModule;
      if (!wasmModule || typeof wasmModule.setActiveConstraint !== 'function') {
        logger.error(LOG_CATEGORIES.WASM, 'WASM module not ready');
