...
class EventManager {
  ...
  private preventDefaultWhenFocused: string[];
  private earlyKeyboardListenersAttached: boolean;

  constructor(editor: Editor, fileOperations: FileOperations | null = null) {
    this.editor = editor;
  ...
  setupFocusManagement(): void {
    ...
    this.addEventListener('focus-return-request', (_event: CustomEvent) => {
   ...
   */
  handleGlobalFocus(event: FocusEvent): void {
    const target = event.target as Element;

    // Check if focus is on editor canvas
    if (this.isEditorElement(target)) {
      this.handleEditorFocus(target as HTMLElement);
    } else {
   ...
   */
  handleGlobalBlur(event: FocusEvent): void {
   ...
   * Handle editor focus
   */
  handleEditorFocus(canvas: HTMLElement): void {
   ...
   */
  handleOutsideClick(_event: MouseEvent): void {
   ...
   */
  handleMenuItemClick(target: Element): void {
    const menuItem = target.closest('[data-menu-item]') as HTMLElement | null;
   ...
   * Handle tab clicks
   */
  handleTabClick(target: Element): void {
  ...
  returnFocusToEditor(): void {
    const textarea = document.querySelector('.notation-textarea') as HTMLElement | null;
    if (textarea) {
      textarea.focus();
    }
  }
...
• Editor shows visual focus indication
  ...
  editorFocus(): boolean {
    const editorElement = document.getElementById('notation-editor');
    return editorElement === document.activeElement || (editorElement?.contains(document.activeElement) ?? false);
  }

  /**
   * Check if element is part of the editor
   */
  isEditorElement(element: Element | null): boolean {
  ...
  getFocusableElements(): HTMLElement[] {
    ...
    return Array.from(document.querySelectorAll(selectors.join(', ')))
      .filter(el => (el as HTMLElement).offsetParent !== null) as HTMLElement[]; // Only visible elements
  }

  /**
   * Update focus indicators
   */
  updateFocusIndicators(hasFocus: boolean): void {
   ...
   */
  logFocusEvent(detail: { hasFocus: boolean; element: Element }): void {
    if (detail.hasFocus) {
