...
interface Editor {
  wasmModule?: {
    ...
    exportMIDIDirect: (tpq: number, tempo: number) => Uint8Array;
...
class ExportUI {
  ...
  private fileOperations: unknown;
  private modalElement: HTMLElement | null = null;
  private isOpen: boolean = false;

  constructor(editor: Editor, fileOperations: unknown = null) {
    this.editor = editor;
  ...
  open(): void {
    ...
    const firstButton = this.modalElement?.querySelector<HTMLElement>('button[data-export]');
    if (firstButton) {
   ...
   */
  private createExportButton(option: ExportOption): HTMLButtonElement {
   ...
   */
  private async handleExport(formatId: string): Promise<void> {
    this.close();
   ...
   */
  async export(formatId: string): Promise<ExportResult> {
    if (!this.editor) {
   ...
   * Export as MusicXML
   */
  private async exportMusicXML(baseFilename: string): Promise<ExportResult> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new Error(`MusicXML export failed: ${errorMessage}`);
    }
  }
   ...
   */
  private async exportMIDI(baseFilename: string): Promise<ExportResult> {
    ...
    try {
      ...
      // Convert Uint8Array to Blob (cast needed for TypeScript)
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new Error(`MIDI export failed: ${errorMessage}`);
    }
  }
   ...
   */
  private async exportLilyPondSource(baseFilename: string): Promise<ExportResult> {
   ...
   */
  private async generateLilyPondSource(): Promise<string> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new Error(`LilyPond export failed: ${errorMessage}`);
    }
  }
