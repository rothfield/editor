...
import './font-inspector.js';  // Side-effect import to run initialization
import './markup.js';  // Side-effect import to run initialization

// Declare window extensions (use any for MusicNotationApp to avoid conflict with WASMBridge.ts)
declare global {
  interface Window {
...
class MusicNotationApp {
  editor: MusicNotationEditor | null;
  eventManager: EventManager | null;
  fileOperations: FileOperations | null;
  ui: UI | null;
  exportUI: ExportUI | null;
  preferencesUI: PreferencesUI | null;
  resizeHandle: ResizeHandle | null;
  fontTestUI: FontTestUI | null;
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.fileOperations = new FileOperations(this.editor as any);
      this.eventManager = new EventManager(this.editor as any, this.fileOperations as any);
      this.preferencesUI = new PreferencesUI(this.editor as any);
      this.ui = new UI(this.editor as any, this.fileOperations as any, this.preferencesUI);
      this.exportUI = new ExportUI(this.editor as any, this.fileOperations);
      this.resizeHandle = new ResizeHandle();

      // Pass references to editor so it can access other components
      this.editor.ui = this.ui as any;
   ...
   */
  async handleDOMContentLoaded(): Promise<void> {
    try {
   ...
   * Initialize LilyPond tabs
   */
  initializeLilyPondTabs(): void {
    try {
      if (!this.editor) return;

      // Initialize LilyPond Source tab (displays source code)
      const lilypondSrcTab = new LilyPondTab(this.editor as any);
      ...
      const lilypondPngTab = new LilyPondPngTab(this.editor as any, lilypondRenderer);
   ...
   */
  setupMidiControls(): void {
    const playButton = document.getElementById('midi-play') as HTMLButtonElement | null;
    const pauseButton = document.getElementById('midi-pause') as HTMLButtonElement | null;
    const stopButton = document.getElementById('midi-stop') as HTMLButtonElement | null;
    const tempoInput = document.getElementById('midi-tempo') as HTMLInputElement | null;
    ...
    playButton.addEventListener('click', async () => {
      try {
        ...
        statusSpan.textContent = 'Error: ' + (error as Error).message;
      }
    });
    ...
    // Volume control
    const volumeSlider = document.getElementById('midi-volume') as HTMLInputElement | null;
    ...
    if (volumeSlider && volumeLabel) {
      volumeSlider.addEventListener('input', (e) => {
        ...
        if (this.editor && this.editor.osmdRenderer && this.editor.osmdRenderer.audioPlayer) {
          if (this.editor.osmdRenderer.audioPlayer.playbackSettings) {
  ...
  setupResizeRedraw(): void {
    ...
    this.resizeHandle.setOnResizeEnd(() => {
      try {
        ...
        const activeTabButton = document.querySelector('[data-tab].active') as HTMLElement | null;
        if (!activeTabButton) {
  ...
  setupHotReloadToggle(): void {
    ...
    const waitForHotReload = (): Promise<void> => {
    ...
    waitForHotReload().then(() => {
      ...
      const updateUI = (): void => {
        ...
        (toggleBtn as HTMLElement).style.opacity = enabled ? '1' : '0.6';
        (toggleBtn as HTMLElement).style.color = enabled ? 'inherit' : '#9ca3af';
      };
 ...
 */
class AppFactory {
  static create(): MusicNotationApp {
    const app = new MusicNotationApp();
    return app;
  }

  static async init(): Promise<MusicNotationApp> {
 ...
 */
let appInstance: MusicNotationApp | null = null;
...
const DevUtils = {
   ...
   */
  getAppState(): AppState | { error: string } {
    if (!appInstance) {
    ...
    return {
      ...
      editor: {
        ...
        document: (appInstance.editor as any)?.document || null,
        cursor: (appInstance.editor as any)?.getCursorPosition?.() || 0
      }
    };
  },
  ...
  getPerformanceStats(): unknown {
    ...
    return (appInstance.editor as any).getPerformanceStats?.();
  },

  /**
   * Test musical notation parsing
   */
  async testNotation(notation: string): Promise<void> {
    if (!appInstance || !appInstance.editor) {
    ...
    await (appInstance.editor as any).insertText(notation);
 ...
 */
(window as any).MusicNotationApp = {
  app: () => appInstance,
  utils: DevUtils,
  init: initializeApp
};
